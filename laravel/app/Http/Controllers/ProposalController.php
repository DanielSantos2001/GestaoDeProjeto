...
class ProposalController extends Controller
{
  ...
  public function store(Request $request)
  {
    ...
    $tempMail = $request->emailEmpresa;
    if (str_contains($tempMail, ' - ')) {
      $email = explode(" - ", $request->emailEmpresa);
      $idEmpresa = DB::table('users')->where('USER_MAIL', $email[1])->value('USER_ID');
    } else {
    ...
    if (!empty($idEmpresa)) {
      if (DB::table('users')->where('USER_ID', $idEmpresa)->value('USER_TYPE') == 'empresa') {
        ...
        return redirect()->back()->withErrors(['O email inserido não é de uma empresa.', 'O email inserido não é de uma empresa.']);
      }
    } else {
      return redirect()->back()->withErrors(['O email inserido é inválido.', 'O email inserido é inválido.']);;
    }
  }
  ...
  {
    $mails = DB::table('users')->where('USER_TYPE', 'empresa')->get();
